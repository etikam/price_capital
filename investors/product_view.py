...
from .forms import AchatForm, InvestorForm
from .models import Achat, GainRecord, Investor
...
def initiate_purchass(request, uid):
    ...
    if investor:
        if Achat.objects.filter(buyer=investor, product=product).exists():
            messages.info(
                request,
                "vous avez deja initialisé l'investissement pour ce projet, veuillez le consulter dans votre espace Investissement",
            )
...
def annuler_achat(request, uid):
    ...
    try:
        ...
        messages.success(request, f"L'achat du produit '{product_name}' a été supprimé avec succès.")

    except Achat.DoesNotExist:
        messages.error(request, "L'achat que vous essayez d'annuler n'existe pas ou ne vous appartient pas.")
    except Exception as e:
        messages.error(
            request,
