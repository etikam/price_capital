...
def complete_profile(request, uid):
    ...
    if PhysicalPerson.objects.filter(user=user).exists():
        messages.info(request, "Vous avez deja un profile en tant que Personne physique, si vous voulez le modifier, allez dans votre profile")
        return redirect('home')
    
    if MoralPerson.objects.filter(user=user).exists():
        messages.info(request, "Vous avez deja un profile en tant que Personne Morale, si vous voulez le modifier, allez dans votre profile")
        return redirect('home')
    ...
    if request.method == 'POST':
        ...
        if role == 'PHYSICAL':
            ...
            if physical_form.is_valid():
                ...
                messages.success(request, "Votre profil a été complété avec succès, vous pouvez maintenant vous connecter.")
